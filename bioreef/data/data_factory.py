...
class WaterNetRestorer(nn.Module):
    ...
    def _load_model(self):
        """Lazy-load Water-Net model weights."""
        if self._model is not None:
            return

        try:
            hub_output = torch.hub.load(
                "tnwei/waternet",
                "waternet",
                pretrained=True,
                force_reload=False,
            ...
            if isinstance(hub_output, tuple) and len(hub_output) >= 3:
                self._preprocess, self._postprocess, self._model = hub_output[:3]
                
                # The hub model loads onto CPU by default. We must explicitly push it to the Quadro GPUs.
                if torch.cuda.is_available():
                    self._model = self._model.cuda()
            else:
                raise ValueError("Expected preprocess, postprocess, and model from hub")
                
            self._model.eval()
            logger.info("Water-Net loaded from torch hub (pretrained) with preprocessing hooks attached.")
        except Exception as e:
            logger.warning(
                f"Could not load Water-Net from hub: {e}. "
                "Falling back to identity (no restoration)."
            )
    ...
    def forward(self, image: np.ndarray) -> np.ndarray:
        """
        Restore a single underwater image using the 4-stream WaterNet.
        """
        self._load_model()
        ...
        device = next(self._model.parameters(), torch.tensor(0.0)).device
        
        # Convert BGR -> RGB stringently for tnwei/waternet
        rgb = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)

        if self._preprocess is not None and self._postprocess is not None:
            try:
                # WaterNet demands 4 distinct physical derivations of the frame
                rgb_ten, wb_ten, he_ten, gc_ten = self._preprocess(rgb)
                rgb_ten = rgb_ten.to(device)
                wb_ten = wb_ten.to(device)
                he_ten = he_ten.to(device)
                gc_ten = gc_ten.to(device)
                
                # Fuse the 4 contextual inputs
                out_ten = self._model(rgb_ten, wb_ten, he_ten, gc_ten)
                
                # Postprocess extracts the final array back to (H, W, 3) cleanly
                restored_rgb = self._postprocess(out_ten)[0]
            except Exception as e:
                # Occurs if the raw image is solid color/zero-variance causing NaNs
                logger.debug(f"WaterNet physics breakdown ({e}). Falling back to raw image.")
                return image
        else:
            try:
                # Absolute legacy fallback
                rgb_norm = rgb.astype(np.float32) / 255.0
                tensor = torch.from_numpy(rgb_norm).permute(2, 0, 1).unsqueeze(0).to(device)
                ...
                logger.debug(f"WaterNet legacy breakdown ({e}). Falling back to raw image.")
                return image

        # Return to BGR layout identically aligned with OpenCV standard
